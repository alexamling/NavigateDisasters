...
namespace KinematicVehicleSystem
{
    ...
    public class KinematicController : MonoBehaviour
    {
        ...
        public void Move()
        {
            ...
            {
                if (!Rb.constraints.HasFlag(RigidbodyConstraints.FreezeRotation))
                {
                    Rb.MoveRotation(SteerTarget);
                }
            }
        ...
        public void UpdateMotorVolume()
        {
            float volume = (MoveTarget - transform.position).magnitude * 2 + (SteerTarget.eulerAngles - transform.rotation.eulerAngles).magnitude / 2;
            volume = Mathf.Clamp(volume, .1f, 1);
            Audio.volume = volume;
        }
